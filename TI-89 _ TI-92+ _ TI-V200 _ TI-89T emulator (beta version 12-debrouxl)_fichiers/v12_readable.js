...
var hex_digits='0123456789ABCDEF';function to_hex(number,digits){var s="";if(number<0){number=-number;digits--;s="-";}
while(digits--){var digit=number%16;number=(number-digit)/16;s=hex_digits[digit]+s;}
return s;}
function to_hex2(number,digits){var s="";while(digits--){var digit=number&15;number=number>>4;s=hex_digits[digit]+s;}
return s;}
...
function memory_dump(address,size,stride){address&=0xFFFFFF;size&=0xFFFFFF;stride&=0xFFFFFF;var end=address+size;var str=to_hex(address,6)+"\t";var i=0;while(address<end){if(i==stride){str+="\n"+to_hex(address,6)+"\t";i=0;}
str+=to_hex(rb(address),2)+" ";address++;i++;}
stdlib.console.log(str);}
function ROM_CALL(id){id&=0xFFFF;return rl(state.jmp_tbl+4*id);}
function HeapTable(){if(state.pedrom&&state.ram[0x30>>>1]<=0x0080){return rl(0x5d58);}
else{if(ROM_CALL(-1)<0x441&&!state.pedrom){return rl(rw(ROM_CALL(0x96)+8));}
else{return ROM_CALL(0x441);}}}
function HeapDeref(id){id&=0xFFFF;return rl(HeapTable()+4*id);}
function HeapSizeAddress(address){if(!state.pedrom){return((rw(address-2)&0x7FFF)-1)<<1;}
else{if(address>=state.ROM_base){return rw(address)+2;}
else{return rl(address-6)-6;}}}
function HeapSize(id){id&=0xFFFF;return HeapSizeAddress(rl(HeapTable()+4*id));}
function PrintHeap(){var address=HeapTable()+4;stdlib.console.log("0\tFFFFFF\tN/A");for(var i=1;i<2000;i++){var handle=rl(address);if(handle!=0){stdlib.console.log(i+"\t"+to_hex(handle,6)+"\t"+to_hex(HeapSizeAddress(handle),6));}
address+=4;}}
function disassemble_indexed_disp(disp){return((disp&0x8000)?"A":"D")+(((disp&0x7000)>>>12)&0x7)+((((disp&0x0800)>>>11)&1)?".L":".W")+")";}
function disassemble_regs_mask(regs8,prefix){var str="";var previous=0;var current;var start=-1;var end=-1;var i;for(i=0;i<8;i++){current=regs8&1;if(previous==0&&current==1)start=i;if(previous==1&&current==0)end=i-1;if(start==end&&start!=-1){str+=prefix+(i-1)+"/";end=-1;start=-1;}
...
return str;}
function disassemble_regs_predec_mask(regs8,prefix){var str="";var previous=0;var current;var start=-1;var end=-1;var i;for(i=0;i<8;i++){current=(regs8&(1<<7))>>7;if(previous==0&&current==1)start=i;if(previous==1&&current==0)end=i-1;if(start==end&&start!=-1){str+=prefix+(i-1)+"/";end=-1;start=-1;}
else if(end>start){str+=prefix+start+"-"+prefix+end+"/";end=-1;start=-1;}
previous=current;regs8<<=1;}
end=i-1;if(end>(start+1)&&start!=-1){str+=prefix+start+"-"+end;}
else if(start>0&&end>0){str+=prefix+start;}
...
return str;}
function disassemble(address,count){if(address&1){stdlib.console.log("Cowardly refusing to disassemble from an odd address");return;}
...
count--;}}
function print_status(){stdlib.console.log('---')
...
function print_status2(){stdlib.console.log('---')
...
function sbcd(dst,src){src&=0xFF;dst&=0xFF;var subtrahend=(src>>>4)*10+(src&0xF);var minuend=(dst>>>4)*10+(dst&0xF);var result=minuend-subtrahend;if(state.sr&0x10)result--;state.sr&=0xFFE4;if(result<0){result=result+100;state.sr|=0x11;}
...
function nbcd(src){src&=0xFF;var result;if(state.sr&0x10){result=153-src;if(result<0)result+=256;}
...
return result;}
function addx(x,y,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;var result=x+y;if(state.sr&0x10)result++;var maskedresult=result>=overflow?result-overflow:result;state.sr&=0xFFE0;if(result==0)state.sr|=4;if(result&neg)state.sr+=8;if(result!=maskedresult)state.sr+=0x11;if(maskedresult<0)maskedresult+=overflow;if(x<neg&&y<neg&&maskedresult>=neg)state.sr+=2;if(x>=neg&&y>=neg&&maskedresult<neg)state.sr+=2;return maskedresult;}
function subx(x,y,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;var result=y-x;if(state.sr&0x10)result--;state.sr&=0xFFE4;if(result<0){result+=overflow;state.sr|=0x11;}
...
function muls(x,y){x=x&0xFFFF;y=y&0xFFFF;if(x>=0x8000)x-=0x10000;if(y>=0x8000)y-=0x10000;var product=x*y;state.sr&=0xFFF0;if(product<0){product+=4294967296;state.sr|=8;}
if(product==0)state.sr|=4;return product;}
function mulu(x,y){x=x&0xFFFF;y=y&0xFFFF;var product=x*y;state.sr&=0xFFF0;if(product>=2147483648)state.sr|=8;if(product==0)state.sr|=4;return product;}
function divu(divisor,dividend){if(divisor==0)fire_cpu_exception(5);divisor&=0xFFFF;var quotient=Math.floor(dividend/divisor)&4294967295;var remainder=(dividend%divisor)&0xFFFF;state.sr&=0xFFF0;if(quotient==0)state.sr|=4;if(quotient&4294901760){if(quotient>=2147483648)state.sr|=8;state.sr|=2;return dividend;}
...
function lsl(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFF0;while(shift>0){if(x&neg)state.sr|=0x11;else state.sr&=0xFFEE;x<<=1;if(x>=overflow){x-=overflow;}
shift--;}
x&=overflow-1;if(x&neg)state.sr|=8
if(x==0)state.sr|=4;return x;}
function asl(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFE1;if(shift>0)state.sr&=0xFFE0;while(shift>0){if(x&neg)state.sr|=0x11;else state.sr&=0xFFEE;var old=x;x<<=1;if(x>=overflow){x-=overflow;}
if((x&neg)!=(old&neg))state.sr|=2;shift--;}
x&=overflow-1;if(x&neg)state.sr|=8
if(x==0)state.sr|=4;return x;}
function lsr(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFE0;while(shift>0){if(x&1)state.sr|=0x11;else state.sr&=0xFFEE;x>>>=1;shift--;}
x&=overflow-1;if(x&neg)state.sr|=8
if(x==0)state.sr|=4;return x;}
function asr(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFF0;if(shift>0)state.sr&=0xFFEF;while(shift>0){if(x&1)state.sr|=0x11;else state.sr&=0xFFEE;if(x&neg){x>>>=1;x|=neg;}
else{x>>>=1;}
shift--;}
x&=overflow-1;if(x&neg)state.sr|=8
if(x==0)state.sr|=4;return x;}
function ror(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFF0;while(shift--){var out=x&1;x>>>=1;if(out)x=x+neg;}
x&=overflow-1;if(x&neg)state.sr|=0x9
if(x==0)state.sr|=4;return x;}
function rol(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFF0;while(shift>0){if(x&neg){x<<=1;x++;}
else{x<<=1;}
if(x>=overflow){x-=overflow;}
shift--;}
x&=overflow-1;if(x&neg)state.sr|=0x8;if(x&1)state.sr|=1;if(x==0)state.sr|=4;return x;}
function roxr(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;while(shift--){var out=x&1;x>>>=1;if(state.sr&0x10)x=x+neg;state.sr=state.sr&0xFFE0;if(out)state.sr+=0x10;}
x&=overflow-1;if(x&neg)state.sr|=0x9
if(x==0)state.sr|=4;if(state.sr&0x10)state.sr|=1;return x;}
function roxl(x,shift,size){var overflow=0x100;if(size==1)overflow=0x10000;if(size==2)overflow=4294967296;var neg=overflow/2;x&=overflow-1;state.sr&=0xFFF0;while(shift>0){var old=x;x<<=1;if(state.sr&0x10)x=x+1;if(old&neg){state.sr|=0x10;}
...
function update_sr(new_sr){if((new_sr^state.sr)&0x2000){var t=state.a7;state.a7=state.a8;state.a8=t;}
state.sr=new_sr&0xA71F;}
function an(reg){switch(reg){case 0:return state.a0;case 1:return state.a1;case 2:return state.a2;case 3:return state.a3;case 4:return state.a4;case 5:return state.a5;case 6:return state.a6;case 7:return state.a7;}}
function dn(reg){switch(reg){case 0:return state.d0;case 1:return state.d1;case 2:return state.d2;case 3:return state.d3;case 4:return state.d4;case 5:return state.d5;case 6:return state.d6;case 7:return state.d7;}}
...
function insert_inst(opcode,code,name){instruction_list+="cpu.t["+opcode+"] = function() { "+code+"};";cpu.n[opcode]=name;}
function insert_inst2(opcode,code,name,count){instruction_list+="cpu.t["+opcode+"] = function() { "+code+"};";cpu.n[opcode]=name;cpu.cycles[opcode]=count;}
function valid_source(mode,reg){return mode<7||reg<=4}
function valid_dest(mode,reg){return mode<7||reg<=1}
function valid_calc_effective_address(mode,reg){return mode==MODE_AREG_INDIRECT||mode==MODE_AREG_OFFSET||mode==MODE_AREG_INDEX||(mode==MODE_MISC&&(reg<=3))}
function size_name(size){if(size==0)return".B"
if(size==1)return".W"
return".L"}
function get_read(size){if(size==0)return"rb"
if(size==1)return"rw"
if(size==2)return"rl"}
function get_write(size){if(size==0)return"wb"
if(size==1)return"ww"
if(size==2)return"wl"}
function amode_name(mode,reg,size){if(mode==MODE_DREG)return"D"+(reg)
...
return"unk"}
function size_imm(size){if(size==0)return" #xx,";else if(size==1)return" #xxx,";else if(size==2)return" #xxxxxx,";}
function read_pc(size,dest,sideeffects){if(size==0){var code="var "+dest+"=rb(state.pc+1);"
return sideeffects?code+"state.pc+=2;":code}
else if(size==1){var code="var "+dest+"=rw(state.pc);"
return sideeffects?code+"state.pc+=2;":code}
else if(size==2){var code="var "+dest+"=rl(state.pc);"
return sideeffects?code+"state.pc+=4;":code}}
function amode_read(mode,reg,size,sideeffects){var increment=size+1;if(increment==3)increment=4;if(increment==1&&reg==7)increment=2;if(mode==MODE_MISC&&reg==MISCMODE_IMM)
return read_pc(size,"s",sideeffects);if(mode==MODE_MISC&&reg==MISCMODE_PC_OFFSET){var code=read_pc(1,"o",sideeffects);code+="var a=state.pc+ewl(o)-2;"
code+="var s="+get_read(size)+"(a);"
return code;}
if(mode==MODE_MISC&&reg==MISCMODE_PC_INDEX){var code=read_pc(1,"e",sideeffects)
code+="var a=e&0xFF;"
code+="if(a>127)a-=256;"
code+="a+=state.pc-2;"
code+="var x=(e>>>12)&7;"
...
return code;}
if(mode==MODE_MISC&&reg==MISCMODE_LONG){code=read_pc(2,"a",sideeffects)
code+="var s="+get_read(size)+"(a);"
return code;}
if(mode==MODE_MISC&&reg==MISCMODE_SHORT){code=read_pc(1,"a",sideeffects)
code+="var s="+get_read(size)+"(ewl(a));"
return code;}
if(mode==MODE_AREG_INDIRECT){return"var s="+get_read(size)+"(state.a"+reg+");"}
if(mode==MODE_AREG_POSTINC){var code="var s="+get_read(size)+"(state.a"+reg+");"
if(sideeffects)code+="state.a"+reg+"+="+increment+";"
return code;}
if(mode==MODE_AREG_PREDEC){if(sideeffects)
return"state.a"+reg+"-="+increment+";"+"var s="+get_read(size)+"(state.a"+reg+");"
else
return"var s="+get_read(size)+"(state.a"+reg+"-"+increment+");"}
if(mode==MODE_AREG_OFFSET){var code=read_pc(1,"o",sideeffects)
code+="var a=state.a"+reg+"+ewl(o);"
code+="var s="+get_read(size)+"(a);"
return code;}
if(mode==MODE_AREG_INDEX){var code=read_pc(1,"e",sideeffects)
code+="var a=e&255;"
code+="if (a>=128)a-=256;"
code+="a+=state.a"+reg+";"
code+="var x=(e>>>12)&7;"
...
function effective_address_calc(mode,reg){var code="fire_cpu_exception(4);"
if(mode==MODE_MISC&&reg==MISCMODE_PC_OFFSET){code=read_pc(1,"o",true)
code+="var z=state.pc-2+ewl(o);"
code+="if(z>4294967295)z-=4294967296;"}
if(mode==MODE_MISC&&reg==MISCMODE_PC_INDEX){code=read_pc(1,"e",true)
code+="var a=e&0xFF;"
code+="if(a>127)a-=256;"
code+="a+=state.pc-2;"
code+="var x=(e>>>12)&7;"
...
code+="var z=y+a;"
code+="if(z>4294967295)z-=4294967296;"}
if(mode==MODE_AREG_INDEX){code=read_pc(1,"e",true)
code+="var a = e&0xFF;"
code+="if(a>127)a-=256;"
code+="a+=state.a"+reg+";"
code+="var x=(e>>>12)&7;"
...
if(mode==MODE_MISC&&reg==MISCMODE_SHORT){code=read_pc(1,"z",true)
code+="z=ewl(z);"}
if(mode==MODE_AREG_OFFSET){code=read_pc(1,"o",true)
code+="var z=state.a"+reg+"+ewl(o);"
code+="if(z>4294967295)z-=4294967296;"}
if(mode==MODE_AREG_INDIRECT)
code="var z=state.a"+reg+";"
return code}
function set_condition_flags_data(size,s){var code="state.sr &= 65520;"
...
function amode_write(mode,reg,size,data){var increment=size+1;if(increment==3)increment=4;if(increment==1&&reg==7)increment=2;if(mode==MODE_MISC&&reg==MISCMODE_LONG)
...
if(mode==MODE_AREG_INDEX){var code=read_pc(1,"e",true)
code+="var a=e%256;"
code+="if(a>127)a-=256;"
code+="a+=state.a"+reg+";"
code+="var x=(e>>>12)&7;"
...
return"fire_cpu_exception(4);"}
function effective_address_calculation_time(mode,reg,size){if(mode==MODE_DREG||mode==MODE_AREG){return 0;}
...
return 0;}
function build_moveq(){for(var data=0;data<=255;data++){for(var reg=0;reg<8;reg++){var opcode=0x7000+(reg<<9)+data;var code="state.sr &= 65520;";code+="state.d"+reg+" = ";if(data<128){code+=data+"; ";if(data==0)
code+="state.sr |= 4;";}
else
...
function build_addsubq(){for(var offset=-8;offset<9;offset++){if(offset!=0){for(var mode=0;mode<8;mode++){for(var reg=0;reg<8;reg++){for(var size=0;size<3;size++){if(valid_dest(mode,reg)&&(mode!=MODE_AREG||size!=0)){var name="";var opcode=0;if(offset>0){opcode=0x5000+(offset<<9)
if(offset==8)opcode=0x5000
opcode+=(size<<6)+(mode<<3)+reg
name="ADDQ"+size_name(size)+" #"+offset+","+amode_name(mode,reg,0)}
else{opcode=0x5100+((-offset)<<9)
if(offset==-8)opcode=0x5100
opcode+=(size<<6)+(mode<<3)+reg
name="SUBQ"+size_name(size)+" #"+(-offset)+","+amode_name(mode,reg,0)}
var actualsize=(mode==MODE_AREG)?2:size;var cost=(size==2)?(((mode==MODE_DREG)||(mode==MODE_AREG))?4:8):(((mode==MODE_DREG)||(mode==MODE_AREG))?8:12);var code=amode_read(mode,reg,actualsize,false);if(mode==MODE_AREG){code+="var r=s+"+offset+";"
...
function build_conditionals(condition,name,bits){var bcc_opcode=0x6000+(bits<<8)
...
var code="";if(o==0){code="var o=rw(state.pc);"
if(name=="F"){code+=amode_write(4,7,2,"(state.pc+2)")
code+="if(true) {"}
else{code+=condition+"{";}
...
for(var reg=0;reg<8;reg++){for(var mode=0;mode<8;mode++){if(valid_dest(mode,reg)&&mode!=1){var opcode=scc_opcode+reg+(mode<<3)
...
function build_moves(name,size,pattern){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){for(var dstmode=0;dstmode<8;dstmode++){if(size==0&&(dstmode==1||srcmode==1))continue;for(var dstreg=0;dstreg<8;dstreg++){if(valid_source(srcmode,srcreg)&&valid_dest(dstmode,dstreg)){var opcode=pattern+(dstreg<<9)+(dstmode<<6)+(srcmode<<3)+srcreg
...
function build_movep(){for(var opmode=4;opmode<6;opmode++){for(var areg=0;areg<8;areg++){for(var dreg=0;dreg<8;dreg++){var opcode=0x0008+(dreg<<9)+(opmode<<6)+areg
...
var code="state.pc += 2"
insert_inst2(opcode,code,fullname,((opmode&1)?24:16))}}}}
function build_operation(name,size,source,dest){var code="";if(size==0&&name=="ADD")code="var r=addb("+source+","+dest+");"
...
if(name=="OR"||name=="AND"||name=="EOR"){code+="if(r<0)r+=4294967296;"
if(size==0)code+="r&=255;"
if(size==1)code+="r&=65535;"
code+=set_condition_flags_data(size,"r")}
return code;}
function build_calc(name,bits){for(var dreg=0;dreg<8;dreg++){for(var reg=0;reg<8;reg++){for(var mode=0;mode<8;mode++){for(var size=0;size<3;size++){var opcode=bits+(dreg<<9)+(size<<6)+(mode<<3)+reg
...
insert_inst2(opcode,code,iname,cost+effective_address_calculation_time(mode,reg,size))}}}}}}
function build_muldiv(name,bits,calcfunc,cost){for(var dreg=0;dreg<8;dreg++){for(var mode=0;mode<8;mode++){for(var reg=0;reg<8;reg++){if(valid_source(mode,reg)&&mode!=MODE_AREG){var opcode=bits+(dreg<<9)+(mode<<3)+reg
...
function build_bit_operation(name,bits,registercost,memorycost){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(srcmode!=1&&(valid_dest(srcmode,srcreg)||(name=='BTST'&&srcmode==MODE_MISC&&(srcreg==MISCMODE_PC_OFFSET||srcreg==MISCMODE_PC_INDEX)))){for(var dreg=0;dreg<=8;dreg++)
{var opcode,iname,code="";if(dreg==8){opcode=bits+(srcmode<<3)+srcreg;iname=name+" #xxx,"+amode_name(srcmode,srcreg,0)}
else{opcode=bits+(srcmode<<3)+srcreg-0x700+(dreg<<9);iname=name+" D"+dreg+","+amode_name(srcmode,srcreg,0)}
if(dreg==8)
code=read_pc(1,"b",true)
if(srcmode<=1){if(dreg==8)
code+="b&=31;"
else
...
if(name!="BTST"){if(srcmode<=1){if(name=="BCLR")code+="s&=(4294967295-(1<<b));"
if(name=="BSET")code+="s|=(1<<b);"
if(name=="BCHG")code+="s^=(1<<b);"
code+="if(s<0)s+=4294967296;"
code+=amode_write(srcmode,srcreg,2,"s")}
else{if(name=="BCLR")code+="s&=(255-(1<<b));"
if(name=="BSET")code+="s|=(1<<b);"
if(name=="BCHG")code+="s^=(1<<b);"
code+=amode_write(srcmode,srcreg,0,"s")}}
insert_inst(opcode,code,iname)}}}}}
function build_cmp(){for(var size=0;size<3;size++){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){for(var firstreg=0;firstreg<8;firstreg++){if(valid_source(srcmode,srcreg)){var opcode=0xB000+(firstreg<<9)+(size<<6)+(srcmode<<3)+srcreg;var iname="CMP"+size_name(size)+" "+amode_name(srcmode,srcreg,size)+",D"+firstreg
var code=amode_read(srcmode,srcreg,size,true)
...
function build_adest(){for(var areg=0;areg<8;areg++){for(var srcreg=0;srcreg<8;srcreg++){for(var srcmode=0;srcmode<8;srcmode++){for(var size=1;size<3;size++){if(valid_source(srcmode,srcreg)){var opcode=0xB0C0+(areg<<9)+((size-1)<<8)+(srcmode<<3)+srcreg
...
code+=amode_write(1,areg,2,"r")
insert_inst2(opcode,code,iname,cost)}}}}}}
function build_shifts(name,mask,altmask,namelower){for(var reg=0;reg<8;reg++){for(var size=0;size<3;size++){for(var shift=0;shift<8;shift++){for(var mm=0;mm<2;mm++){var actualshift=shift==0?8:shift;var iname="";var opcode=mask+0x20+(size<<6)+reg+(shift<<9);var shiftamount;var cost=(size==2)?8:6;if(mm==0){opcode=opcode-0x20;iname=name+size_name(size)+" #"+actualshift+",D"+reg
shiftamount=actualshift}
else{iname=name+size_name(size)+" D"+shift+",D"+reg
...
insert_inst2(opcode,code,iname,cost)}}}}
for(var reg=0;reg<8;reg++){for(var mode=0;mode<8;mode++){if(valid_dest(mode,reg)&&mode!=MODE_DREG&&mode!=MODE_AREG){var opcode=altmask+(mode<<3)+reg;var iname=name+".W "+amode_name(mode,reg,1)
...
function build_immediate(name,mask,operation){for(var reg=0;reg<8;reg++){for(var mode=0;mode<8;mode++){for(var size=0;size<3;size++){if((valid_dest(mode,reg)&&mode!=MODE_AREG)||(mode==MODE_MISC&&reg==4&&size<2&&operation!="")){var opcode=mask+(size<<6)+(mode<<3)+reg
...
if(mode==MODE_MISC&&reg==4){if(size==1)code+="if(state.sr&0x2000==0)fire_cpu_exception(8);";if(size==0&&name=="ANDI")code+="m|=0xFF00;"
code+="update_sr(state.sr"+operation.substring(7,8)+"m);"
insert_inst2(opcode,code,iname,20)}
else{code+=amode_read(mode,reg,size,false)
if(operation!=""){code+=operation;code+=set_condition_flags_data(size,"r")}
else{code+="var r="+name.substring(0,3).toLowerCase()+size_name(size).substring(1,2).toLowerCase()+"(m,s);"}
code+=amode_write(mode,reg,size,"r")
insert_inst2(opcode,code,iname,cost+effective_address_calculation_time(mode,reg,size))}}}}}}
function build_ext(name,bits){for(var src=0;src<8;src++){for(var dst=0;dst<8;dst++){for(var size=0;size<3;size++){for(var mem=0;mem<2;mem++){var opcode=bits+(dst<<9)+(size<<6)+(mem<<3)+src
...
insert_inst2(opcode,code,iname,cost)}}}}}
function build_not_neg_clr_tst_tas(){for(var size=0;size<3;size++){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(valid_dest(srcmode,srcreg)&&srcmode!=MODE_AREG){var opcode=0x4600+(size<<6)+(srcmode<<3)+srcreg;var iname="NOT"+size_name(size)+" "+amode_name(srcmode,srcreg,size)
...
if(size==0){opcode=0x4ac0+(srcmode<<3)+srcreg;iname="TAS.B"+" "+amode_name(srcmode,srcreg,0)
...
function build_lea(){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){for(var reg=0;reg<8;reg++){if(valid_calc_effective_address(srcmode,srcreg)){var opcode=0x41C0+(reg<<9)+(srcmode<<3)+srcreg;var iname="LEA "+amode_name(srcmode,srcreg,1)+",A"+reg
...
insert_inst(opcode,code,iname)}}}}}
function build_cmpi(){for(var size=0;size<3;size++){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(valid_dest(srcmode,srcreg)){var opcode=0xC00+(size<<6)+(srcmode<<3)+srcreg;var iname="CMPI"+size_name(size)+size_imm(size)+amode_name(srcmode,srcreg,size)
...
insert_inst2(opcode,code,iname,cost+effective_address_calculation_time(srcmode,srcreg,size))}}}}}
function build_movem(){for(var reg=0;reg<8;reg++){for(var mode=0;mode<8;mode++){for(var size=1;size<3;size++){var actualsize=size*2
if(mode==MODE_AREG_INDIRECT||mode==MODE_AREG_POSTINC||mode==MODE_AREG_OFFSET||mode==MODE_AREG_INDEX||(mode==MODE_MISC&&(reg==MISCMODE_SHORT||reg==MISCMODE_LONG||reg==MISCMODE_PC_OFFSET||reg==MISCMODE_PC_INDEX))){var opcode=0x4c80+((size-1)<<6)+(mode<<3)+reg
var iname="MOVEM"+size_name(size+1)+" "+amode_name(mode,reg,size)+",regs"
var code=read_pc(1,"regs",true)
if(mode==MODE_AREG_POSTINC)
code+="var newval = load_multiple_postinc(state.a"+reg+", regs, "+size+"); state.a"+reg+" = newval;";else{code+=effective_address_calc(mode,reg);code+="load_multiple(z,regs,"+size+");"}
insert_inst(opcode,code,iname)}
if(mode==MODE_AREG_INDIRECT||mode==MODE_AREG_PREDEC||mode==MODE_AREG_OFFSET||mode==MODE_AREG_INDEX||(mode==MODE_MISC&&(reg==MISCMODE_SHORT||reg==MISCMODE_LONG))){var opcode=0x4880+((size-1)<<6)+(mode<<3)+reg
var iname="MOVEM"+size_name(size)+" regs,"+amode_name(mode,reg,size)
var code=read_pc(1,"regs",true)
if(mode==MODE_AREG_PREDEC){iname=iname.replace("regs","regspredec");code+="var newval = store_multiple_predec(state.a"+reg+", regs, "+size+"); state.a"+reg+" = newval;";}
else{code+=effective_address_calc(mode,reg)
code+="store_multiple(z, regs, "+size+");"}
insert_inst(opcode,code,iname)}}}}}
function build_cmpm(){for(var src=0;src<8;src++){for(var dest=0;dest<8;dest++){for(var size=0;size<3;size++){var opcode=0xB108+(dest<<9)+(size<<6)+src
...
function build_bcd(){for(var src=0;src<8;src++){for(var dest=0;dest<8;dest++){for(var m=1;m>=0;m--){for(var sub=0;sub<=1;sub++){var operation=sub==0?"ABCD":"SBCD"
var opcode=0x8100+(dest<<9)+src
if(operation=="ABCD")opcode+=0x4000
var iname=""
var cost=0;if(m!=0){opcode+=8
iname=operation+" -(A"+src+"),-(A"+dest+")"
cost=18;}
else{iname=operation+" D"+src+",D"+dest
cost=6;}
var code=""
if(m!=0){code=amode_read(MODE_AREG_PREDEC,src,0,true)
code+="var other = s;"
code+=amode_read(MODE_AREG_PREDEC,dest,0,true)
code+=amode_write(MODE_AREG_INDIRECT,dest,0,operation.toLowerCase()+"(s,other)")}
else{code="state.d"+dest+"+="+operation.toLowerCase()+"(state.d"+dest+",state.d"+src+")-state.d"+dest+"&0xFF;"}
insert_inst2(opcode,code,iname,cost)}}}}
for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(valid_dest(srcmode,srcreg)&&srcmode!=MODE_AREG){opcode=0x4800+(srcmode<<3)+srcreg;iname="NBCD "+amode_name(srcmode,srcreg,0)
...
function build_movesrccr(){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(valid_source(srcmode,srcreg)&&srcmode!=MODE_AREG){var opcode=0x46C0+(srcmode<<3)+srcreg;var iname="MOVE "+amode_name(srcmode,srcreg,1)+",SR"
...
if(valid_dest(srcmode,srcreg)&&srcmode!=MODE_AREG){var opcode=0x40C0+(srcmode<<3)+srcreg;var iname="MOVE SR,"+amode_name(srcmode,srcreg,1)
...
function build_exchange(xtype,ytype,bits){for(var x=0;x<8;x++){for(var y=0;y<8;y++){var opcode=bits+(x<<9)+y
...
function build_jmpjsr(){for(var mode=0;mode<8;mode++){for(var reg=0;reg<8;reg++){if(valid_calc_effective_address(mode,reg)){for(var jsr=1;jsr>=0;jsr--){var opcode=0x4EC0+(mode<<3)+reg-jsr*0x40;var iname=(jsr==1?"JSR ":"JMP ")+amode_name(mode,reg,0)
var code=effective_address_calc(mode,reg)
if(jsr==1)
code+=amode_write(4,7,2,"state.pc")
code+="state.pc=z;"
insert_inst(opcode,code,iname)}}}}}
function build_pea(){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){if(valid_calc_effective_address(srcmode,srcreg)){var opcode=0x4840+(srcmode<<3)+srcreg;var iname="PEA "+amode_name(srcmode,srcreg,0)
insert_inst(opcode,effective_address_calc(srcmode,srcreg)+amode_write(4,7,2,"z"),iname)}}}}
function build_swap(){for(var reg=0;reg<8;reg++){var code="var l = state.d"+reg+"&65535;"
code+="var h = state.d"+reg+" >>> 16;"
code+="state.d"+reg+" = (l * 65536) + h;"
insert_inst2(0x4840+reg,code,"SWAP D"+reg,4)}}
function build_chk(){for(var srcmode=0;srcmode<8;srcmode++){for(var srcreg=0;srcreg<8;srcreg++){for(var reg=0;reg<8;reg++){if(valid_dest(srcmode,srcreg)&&srcmode!=MODE_AREG){var opcode=0x4180+(reg<<9)+(srcmode<<3)+srcreg;var iname="CHK.W "+amode_name(srcmode,srcreg,1)+",D"+reg
...
insert_inst(opcode,code,iname)}}}}}
function build_initial_instructions_handlers(){var i;for(i=0;i<0xA000;i++){cpu.t[i]=unhandled_instruction;cpu.n[i]='UNKNOWN';cpu.cycles[i]=0;}
...
function build_all_instructions(){build_initial_instructions_handlers();build_moveq();build_addsubq();build_moves("MOVE.L",2,0x2000);build_moves("MOVE.W",1,0x3000);build_moves("MOVE.B",0,0x1000);build_movep();build_conditionals("if(true)","T",0)
...
build_all_instructions();function read_hreg(reg){switch(reg){case 0x600000:{return 0x04;}
...
function write_hreg(reg,value){switch(reg){case 0x600000:{break;}
...
case 0x600014:{break;}
case 0x600015:{state.interrupt_control=value;switch((state.interrupt_control>>4)&0x3){case 0:state.interrupt_rate=0x20;break;case 1:state.interrupt_rate=0x200;break;case 2:state.interrupt_rate=0x1000;break;case 3:state.interrupt_rate=0x40000;break;}
break;}
case 0x600016:{break;}
...
build_movem_handlers();function initemu(){state.sr=0;stdlib.console.log("JS TI-68k emulator by PatrickD, Lionel Debroux et. al starting");if(!checkemu()){stdlib.console.log("Emulation checks failed");return;}
if(!detect_calculator_model()){stdlib.console.log("Couldn't detect calculator model");ui.display_no_rom_loaded();return;}
ui.setCalculatorModel(state.calculator_model);link.setCalculatorModel(state.calculator_model);ui.initemu();initialize_calculator();main_interval_timer_id=stdlib.setInterval(emu_main_loop,state.main_interval_timer_interval);for(var key=0;key<80;key++)state.keystatus[key]=0;ui.initkeyhandlers();};function setKey(keynumber,status){state.keystatus[keynumber]=status;}
function setONKeyPressed(){state.port_60001A=0x00;raise_interrupt(6);}
function setONKeyReleased(){state.port_60001A=0x02;}
function detect_calculator_model(){if(typeof(state.rom)!=="object"){return false;}
...
var reset=false;function initialize_calculator(){reset_calculator();reset();}
function reset_calculator(){if(state.erase_ram_upon_reset){for(var b=0;b<131072;b++){state.ram[b]=0;}}
...
function raise_interrupt(i){if(state.stopped){if(i==6||i==7){stdlib.console.log("Resuming from stop due to AUTO_INT_6 or AUTO_INT_7, wakemask="+to_hex(state.wakemask,2));state.stopped=false;}
else{if(state.wakemask&(1<<(i-1))){state.stopped=false;}}}
state.pending_ints|=1<<i;}
function fire_cpu_exception(e){if(e>=25&&e<=30){var interrupt_level=e-24;var current_level=(state.sr&0x700)>>8;if(current_level>=interrupt_level){return;}}
...
function timer_interrupts(){state.osc2_counter+=32;if(state.osc2_counter>=0x1000000)state.osc2_counter-=0x1000000;if((state.interrupt_control&0x80)==0){if(state.interrupt_control&2){if((state.osc2_counter&0x7FF)==0)
raise_interrupt(1);if(((state.osc2_counter%state.interrupt_rate)==0)&&(state.interrupt_control&8)){if(state.timer_current==0)
state.timer_current=state.timer_min;else
state.timer_current++;if(state.timer_current>=256){state.timer_current=0;raise_interrupt(5);}}}
if((state.osc2_counter&0x7FFFF)==0&&(state.interrupt_control&4)&&((state.hardware_model==1&&(state.interrupt_control&2))||(state.hardware_model>1))){raise_interrupt(3);}}}
function execute_instructions(number){var t=cpu.t;var cycles=cpu.cycles;for(var inner=0;inner<number;inner++){state.prev_pc=state.pc;state.reading_instruction=0;state.current_instruction=rw(state.pc);if(typeof(state.current_instruction)==="undefined"){print_status();stdlib.console.log(to_hex(state.pc,9));stdlib.clearInterval(main_interval_timer_id);throw"e";}
...
mask>>=1;}}}}
function emu_main_loop(){if(state.unhandled_count>=10)return;var starttime=(new Date).getTime();var started=false;for(var outer=0;outer<state.screen_height*2;outer++){if(!state.stopped){execute_instructions(64);}
...
toggle_framesync();var endtime=(new Date).getTime();state.total_time+=(endtime-starttime);state.frames_counted++;if(state.frames_counted==1000){ui.set_title("Average milliseconds for the last 1000 frames is "+(state.total_time/1000));state.total_time=state.frames_counted=0;}
if(newromready){handle_newromready();}
if(newfileready){handle_newfileready();}
if(newflashfileready){handle_newflashfileready();}}
function handle_newromready(){var inputrom=newromready.result;newromready=false;var buf=new Uint8Array(inputrom);if(inputrom.byteLength==0x200000||inputrom.byteLength==0x400000){stdlib.console.log("Processing plain ROM image");rom=new Uint16Array(inputrom.byteLength/2);for(var x=0;x<inputrom.byteLength;x+=2){state.rom[x/2]=buf[x]*256+buf[x+1];}
initemu();}
else{stdlib.console.log("Processing TIB/9XU image");var start=0;if(buf[0]==0x2A&&buf[1]==0x2A&&buf[2]==0x54&&buf[3]==0x49&&buf[4]==0x46&&buf[5]==0x4C&&buf[6]==0x2A&&buf[7]==0x2A){for(var test=0;test<inputrom.byteLength-8;test++){if(buf[test]==0x62&&buf[test+1]==0x61&&buf[test+2]==0x73&&buf[test+3]==0x65&&buf[test+4]==0x63&&buf[test+5]==0x6f&&buf[test+6]==0x64&&buf[test+7]==0x65){start=test+0x3d;break;}}}
...
for(var x=start;x<inputrom.byteLength;x+=2){state.rom[offset]=buf[x]*256+buf[x+1];offset++;}
...
function handle_newfileready(){var buf;if(typeof(newfileready)=="object"){if(newfileready instanceof Array){buf=newfileready;}
else{buf=new Uint8Array(newfileready.result);}}
newfileready=false;var varname=new Array();for(var x=0x0A;x<0x12;x++){if(buf[x]==0)break;varname.push(buf[x]);}
varname.push(0x5c);for(var x=0x40;x<0x48;x++){if(buf[x]==0)break;varname.push(buf[x]);}
...
function handle_newflashfileready(){var buf;if(typeof(newflashfileready)=="object"){if(newflashfileready instanceof Array){buf=newflashfileready;}
else{buf=new Uint8Array(newflashfileready.result);}}
newflashfileready=false;var varname=new Array();for(var x=0x11;x<0x19;x++){if(buf[x]==0)break;varname.push(buf[x]);}
...
function loadrom(infile){stdlib.console.log("starting to read file "+infile.name);var extension=infile.name.toLowerCase().substr(-4);if((infile.size==0x200000||infile.size==0x400000)&&extension==".rom"){stdlib.console.log("Loading as plain ROM");var reader=new FileReader();reader.onload=function(){newromready=reader;state.unhandled_count=0;handle_newromready();};reader.readAsArrayBuffer(infile);}
if(infile.size>=1024&&infile.size<0x400000&&(extension==".tib"||extension==".9xu"||extension==".89u"||extension==".v2u")){stdlib.console.log("Starting to load as TIB / OS upgrade");var reader=new FileReader();reader.onload=function(){newromready=reader;state.unhandled_count=0;handle_newromready();};reader.readAsArrayBuffer(infile);}
...
function check_muls(){var result;state.sr=0;result=muls(0x0,0x0);if(result!=0||state.sr!=4){stdlib.console.log("muls 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
...
return true;}
function check_mulu(){var result;state.sr=0;result=mulu(0x0,0x0);if(result!=0||state.sr!=4){stdlib.console.log("mulu 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
...
return true;}
function check_divs(){var result;state.sr=0;result=divs(0x10,0x12345678);if(result!=0x12345678||(state.sr&3)!=2){stdlib.console.log("divs 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
...
return true;}
function check_lsl(){var result;state.sr=0;result=lsl(0x80000000,1,2);if(result!=0x00000000||state.sr!=0x15){stdlib.console.log("lsl 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
return true;}
function check_asl(){var result;state.sr=0;result=asl(0x80000000,1,2);if(result!=0x00000000||state.sr!=0x17){stdlib.console.log("asl 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
return true;}
function check_lsr(){var result;return true;}
function check_asr(){var result;return true;}
function check_ror(){var result;return true;}
function check_rol(){var result;state.sr=0;result=rol(0x80000000,1,2);if(result!=0x00000001||state.sr!=0x1){stdlib.console.log("rol 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
...
return true;}
function check_roxr(){var result;return true;}
function check_roxl(){var result;state.sr=0;result=roxl(0x80000000,1,2);if(result!=0x00000000||state.sr!=0x15){stdlib.console.log("roxl 0 "+to_hex(state.sr,4)+" "+to_hex(result,16));return false;}
...
return true;}
function checkemu(){return check_ext()&&check_subb()&&check_cmpb()&&check_addb()&&check_subw()&&check_cmpw()&&check_addw()&&check_subl()&&check_cmpl()&&check_addl()&&check_abcd()&&check_sbcd()&&check_nbcd()&&check_addx()&&check_subx()&&check_muls()&&check_mulu()&&check_divu()&&check_divs()&&check_lsl()&&check_asl()&&check_lsr()&&check_asr()&&check_ror()&&check_rol()&&check_roxr()&&check_roxl();};function setRom(newrom){state.rom=newrom;}
...
function pause_emulator(){stdlib.clearInterval(main_interval_timer_id);}
function increase_emulator_speed(){if(state.main_interval_timer_interval>1){state.main_interval_timer_interval--;stdlib.console.log("Setting main timer interval to "+state.main_interval_timer_interval+" ms.");stdlib.clearInterval(main_interval_timer_id);main_interval_timer_id=stdlib.setInterval(emu_main_loop,state.main_interval_timer_interval);}}
function decrease_emulator_speed(){state.main_interval_timer_interval++;stdlib.console.log("Setting main timer interval to "+state.main_interval_timer_interval+" ms.");stdlib.clearInterval(main_interval_timer_id);main_interval_timer_id=stdlib.setInterval(emu_main_loop,state.main_interval_timer_interval);}
function resume_emulator(){main_interval_timer_id=stdlib.setInterval(emu_main_loop,state.main_interval_timer_interval);}
function toggle_framesync(){state.port_70001D^=0x80;}
function apiversion(){return 1;}
...
function TI68kEmulatorLinkModule(stdlib){var emu=false;var ui=false;var calculator_model=1;var link_incoming_queue=new Array();var link_outgoing_queue=new Array();var link_config=1;var transmit_finished=false;var reset_upon_ack_with_len=true;var link_recv_varsize=0;var link_recv_vartype=0;var link_recv_varname="";var link_recv_foldername="";var link_recv_data=new Array();var link_recv_mode=0;var link_dirlist_vars=new Array();var link_dirlist_folders=new Array();var link_dirlist_apps=new Array();var link_dirlist_curidx=0;var link_pending_keys=new Array();var link_interval_timer_id=0;function _save_state(){var emustate=new Object();return emustate;}
function _restore_state(linkstate){if(typeof(linkstate)==="object"){stdlib.clearInterval(link_interval_timer_id);}
else{stdlib.console.log("Refusing to restore state from something not an object / from an object without the expected sub-objects");}}
function setUI(newui){ui=newui;}
function setEmu(newemu){emu=newemu;}
function setCalculatorModel(model){calculator_model=model;}
function compute_link_status(){var status=0;if(link_incoming_queue.length>0&&typeof(link_incoming_queue[0])=="number"){status|=0x32;}
else if(link_config&2){status|=0x50;}
return status;}
function read_byte(){if(link_incoming_queue.length>0&&typeof(link_incoming_queue[0])=="number"){return link_incoming_queue.shift();}
else{return 0;}}
function write_byte(value){link_outgoing_queue.push(value);transmit_finished=true;}
function reset_arrays(){link_incoming_queue=new Array();link_outgoing_queue=new Array();}
function dump_incoming_queue(header){var dump=header;for(var y=0;y<link_incoming_queue.length;y++){if(typeof(link_incoming_queue[y])=="number"){dump+=emu.to_hex(link_incoming_queue[y],2)+" ";}
else{dump+=link_incoming_queue[y]+" ";}}
stdlib.console.log(dump);}
function dump_outgoing_queue(header){var dump=header;for(var y=0;y<link_outgoing_queue.length;y++){if(typeof(link_outgoing_queue[y])=="number"){dump+=emu.to_hex(link_outgoing_queue[y],2)+" ";}
else{dump+=link_outgoing_queue[y]+" ";}}
stdlib.console.log(dump);}
function ti89_send_ACK(){link_incoming_queue.push(8,0x56,0,0);}
function ti89_recv_ACK(){link_incoming_queue.push('WAIT_ACK');}
function ti89_send_CTS(){link_incoming_queue.push(8,0x09,0,0);}
function ti89_recv_CTS(){link_incoming_queue.push('WAIT_CTS');}
function ti89_send_CNT(){link_incoming_queue.push(8,0x78,0,0);}
function ti89_recv_CNT(){link_incoming_queue.push('WAIT_CNT');}
function ti89_send_EOT(){link_incoming_queue.push(8,0x92,0,0);}
function ti89_send_KEY(keycode){link_incoming_queue.push(8,0x87);link_incoming_queue.push(keycode&0xFF);link_incoming_queue.push((keycode>>>8)&0xFF);}
function ti89_send_XDP(data_section_len,chunk_len,buf,offset,write_both_checksum_and_length){link_incoming_queue.push(8,0x15);var data_checksum=0;link_incoming_queue.push(data_section_len%256,data_section_len>>>8);if(write_both_checksum_and_length){link_incoming_queue.push(0,0,0,0);link_incoming_queue.push((chunk_len>>>8)&0xFF,chunk_len%256);data_checksum=(chunk_len%256)+((chunk_len>>>8)&0xFF);}
for(var x=offset;x<offset+chunk_len;x++){link_incoming_queue.push(buf[x]);data_checksum+=buf[x];}
link_incoming_queue.push(data_checksum%256,(data_checksum>>>8)%256);}
function ti89_recv_XDP(){link_incoming_queue.push('WAIT_XDP');}
function ti89_recv_VAR(){link_incoming_queue.push('WAIT_VAR');}
function ti89_send_REQ(length,varname,vartype){link_incoming_queue.push(8,0xA2);if(typeof(varname)=="string"){var bytes=new Array();for(var i=0;i<varname.length;++i){bytes.push(varname.charCodeAt(i)&0xFF);}
varname=bytes;}
var header_len=varname.length+6;if(vartype==0x18){header_len++;}
link_incoming_queue.push(header_len,0);link_incoming_queue.push(length%256,(length>>>8)&0xFF,(length>>>16)&0xFF,(length>>>24)&0xFF);link_incoming_queue.push(vartype);link_incoming_queue.push(varname.length);var header_checksum=varname.length+vartype+(length%256)+((length>>>8)&0xFF)+((length>>>16)&0xFF)+((length>>>24)&0xFF);for(var x=0;x<varname.length;x++){link_incoming_queue.push(varname[x]);header_checksum+=varname[x];}
link_incoming_queue.push(header_checksum%256,header_checksum>>>8);}
function ti89_send_RTS(length,varname,vartype){link_incoming_queue.push(8,0xC9);var header_len=varname.length+6+1;link_incoming_queue.push(header_len,0);link_incoming_queue.push(length%256,(length>>>8)&0xFF,(length>>>16)&0xFF,(length>>>24)&0xFF);link_incoming_queue.push(vartype);link_incoming_queue.push(varname.length);var header_checksum=varname.length+vartype+(length%256)+((length>>>8)&0xFF)+((length>>>16)&0xFF)+((length>>>24)&0xFF);for(var x=0;x<varname.length;x++){link_incoming_queue.push(varname[x]);header_checksum+=varname[x];}
link_incoming_queue.push(0);link_incoming_queue.push(header_checksum%256,header_checksum>>>8);}
function sendfile(varname,vartype,buf,data_len,offset,write_both_checksum_and_length){var data_len_full=data_len;if(write_both_checksum_and_length){data_len_full+=2;}
...
function sendkey(keycode){ti89_send_KEY(keycode);ti89_recv_ACK();stdlib.console.log("finished processing for sending key");dump_incoming_queue("Incoming: "+link_incoming_queue.length+" (pseudo-)bytes\n");}
...
function sendkeys(keyarray){var newarray=new Array();while(keyarray.length>0){console.log("length is "+keyarray.length);var item=keyarray.shift();if(typeof(item)=='number'){console.log("found number "+item);newarray.push(item);}
...
var MODE_RECVFILE=0;var MODE_RECVFILE_NS=1;var MODE_DIRLIST_ROOT=2;var MODE_DIRLIST_FOLDER=3;function recvfile_requestchunk(){ti89_send_ACK();ti89_send_CTS();ti89_recv_ACK();ti89_recv_XDP();ti89_send_ACK();ti89_recv_CNT();}
function recvfile(varname,vartype){link_reset_recv_vars();ti89_send_REQ(0,varname,vartype);ti89_recv_ACK();link_recv_mode=MODE_RECVFILE;_recvfile_ns();}
function recvfile_ns(){link_reset_recv_vars();link_recv_mode=MODE_RECVFILE_NS;_recvfile_ns();}
function dirlist(){link_reset_recv_vars();link_reset_dirlist_vars();ti89_send_REQ(0x1F000000,"",0x1A);ti89_recv_ACK();link_recv_mode=MODE_DIRLIST_ROOT;_recvfile_ns();}
function dirlist_folder(foldername){ti89_send_REQ(0x1B000000,foldername,0x1A);ti89_recv_ACK();link_recv_mode=MODE_DIRLIST_FOLDER;_recvfile_ns();}
function _recvfile_ns(){ti89_recv_VAR();recvfile_requestchunk();stdlib.console.log("finished processing for receiving variable / dirlist (first chunk)");dump_incoming_queue("Incoming: "+link_incoming_queue.length+" (pseudo-)bytes\n");}
function link_reset_recv_vars(){link_recv_varsize=0;link_recv_vartype=0
link_recv_varname="";link_recv_foldername="";link_recv_data=new Array();}
function link_reset_dirlist_vars(){link_dirlist_vars=new Array();link_dirlist_folders=new Array();link_dirlist_apps=new Array();link_dirlist_curidx=0;}
function link_reset_state(packettype){stdlib.console.log("Receiving "+packettype+" failed, resetting link state !");link_incoming_queue=new Array();link_outgoing_queue=new Array();link_reset_recv_vars();link_reset_dirlist_vars();emu.raise_interrupt(6);}
function link_magic_number(){if(link_recv_vartype>=35)return"**TIFL**";if(calculator_model==1||calculator_model==9)return"**TI89**";else return"**TI92P*";}
function link_build_output_file(){var output_file=new Array();var magic=link_magic_number();for(var i=0;i<magic.length;i++){output_file.push(magic.charCodeAt(i));}
...
function process_recv_CNTEOT(x){dump_outgoing_queue("WAIT_CNT Before: ");var packet_type=link_outgoing_queue[x+1];var computed_checksum=0;for(var i=4;i<(link_recv_varsize&0xFFFF)+4;i++){link_recv_data[i-4]=link_outgoing_queue[i];computed_checksum+=link_outgoing_queue[i];}
...
function link_handling(){var link_status=compute_link_status();if((link_config&0x40)==0)
{if(((link_config&5)&&link_incoming_queue.length>0&&typeof(link_incoming_queue[0])=="number")||((link_config&6)==6)){emu.raise_interrupt(4);}}
if(link_incoming_queue.length>0){if(link_incoming_queue[0]=='WAIT_ACK'){for(var x=0;x+4<=link_outgoing_queue.length;x++){if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x56)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x56)){if((link_outgoing_queue[x+2]!=0||link_outgoing_queue[x+3]!=0)){link_reset_state("ACK");}
...
else if(link_incoming_queue[0]=='WAIT_CTS'){for(var x=0;x+4<=link_outgoing_queue.length;x++){if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x09)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x09)){if(link_outgoing_queue[x+2]!=0||link_outgoing_queue[x+3]!=0){link_reset_state("CTS");}
...
else if(link_incoming_queue[0]=='WAIT_VAR'){for(var x=0;x+4<=link_outgoing_queue.length;x++){if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x06)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x06)){var length=link_outgoing_queue[x+2]+link_outgoing_queue[x+3]*256;dump_outgoing_queue("WAIT_VAR Before: ");link_outgoing_queue.splice(0,x+4);link_incoming_queue.shift();stdlib.console.log("Eaten an item (VAR) in WAIT_VAR",x);dump_outgoing_queue("After: ");}
else if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x92)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x92)){process_recv_CNTEOT(x);}}}
else if(link_incoming_queue[0]=='WAIT_XDP'){for(var x=0;x+4<=link_outgoing_queue.length;x++){if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x15)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x15)){process_recv_XDP(x);}}}
else if(link_incoming_queue[0]=='WAIT_CNT'){for(var x=0;x+4<=link_outgoing_queue.length;x++){if((link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x78)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x78)||(link_outgoing_queue[x]==0x88&&link_outgoing_queue[x+1]==0x92)||(link_outgoing_queue[x]==0x98&&link_outgoing_queue[x+1]==0x92)){process_recv_CNTEOT(x);}}}}};function buildFileExtensionFromVartype(){var prefix=(calculator_model==1||calculator_model==9)?".89":((calculator_model==8)?".v2":".9x");var suffix="";switch(link_recv_vartype){case 0:suffix="e";break;case 4:suffix="l";break;case 6:suffix="m";break;case 10:suffix="c";break;case 11:suffix="t";break;case 12:suffix="s";break;case 13:suffix="d";break;case 14:suffix="a";break;case 16:suffix="i";break;case 18:suffix="p";break;case 19:suffix="f";break;case 20:suffix="x";break;case 28:suffix="y";break;case 33:suffix="z";break;case 36:suffix="k";break;default:suffix="?";break;}
return prefix+suffix;}
function getFileData(){ui.getFileData(new Blob([link_recv_data],{type:"application/octet-binary"}));}
function get_link_config(){return link_config;}
function set_link_config(value){link_config=value&255;if(value&2==0)transmit_finished=false;}
...
frame++;if(frame==frames_for_averaging)frame=0;};function draw_calcscreen_92P_V200(address,ram){var pixel=frame;if(screen_enabled){for(var y=0;y<128;y++){for(var x=0;x<15;x++){var b=ram[address++];for(var bit=15;bit>=0;bit--){var color=b&0x8000?black_color:white_color;b<<=1;calcscreen[pixel]=color;pixel+=frames_for_averaging;}}}}
else{for(var y=0;y<128*240;y++){calcscreen[pixel]=white_color;pixel+=frames_for_averaging;}}
frame++;if(frame==frames_for_averaging)frame=0;};function output_calcscreen_to_bitmap_scale1(calcscreen,buff){var pixel=0;var p=0;for(var y=0;y<128;y++){for(var x=0;x<240;x++){var color=0;for(var i=0;i<frames_for_averaging;i++){color+=calcscreen[pixel++];}
buff[p]=color;buff[p+1]=color;buff[p+2]=color;p+=4;}}};function output_calcscreen_to_bitmap_scale2(calcscreen,buff){var pixel=0;var p=0;for(var y=0;y<128;y++){for(var x=0;x<240;x++){var color=0;for(var i=0;i<frames_for_averaging;i++){color+=calcscreen[pixel++];}
...
p+=1920;}};function output_calcscreen_to_bitmap_scale3(calcscreen,buff){var pixel=0;var p=0;for(var y=0;y<128;y++){for(var x=0;x<240;x++){var color=0;for(var i=0;i<frames_for_averaging;i++){color+=calcscreen[pixel++];}
...
p+=5760;}};function output_calcscreen_to_bitmap_scale4(calcscreen,buff){var pixel=0;var p=0;for(var y=0;y<3840*128;y+=3840){for(var x=0;x<240;x++){var color=0;for(var i=0;i<frames_for_averaging;i++){color+=calcscreen[pixel++];}
...
p+=11520;}};function draw_screen(address,ram){draw_calcscreen(address,ram);if(screen_scaling_ratio==1){output_calcscreen_to_bitmap_scale1(calcscreen,bitmap.data);context.putImageData(bitmap,0,0);}
...
else if(screen_scaling_ratio==4){output_calcscreen_to_bitmap_scale4(calcscreen,bitmap.data);context.putImageData(bitmap,0,0);}};function create_button(shape,coords,keynumber){var map=document.getElementById(elementid_calcmap);var area=document.createElement(elementid_area);area.shape=shape;area.coords=coords;area.onmousedown=function(){emu.setKey(keynumber,1);}
...
function create_on_button(shape,coords){var map=document.getElementById(elementid_calcmap);var area=document.createElement(elementid_area);area.shape=shape;area.coords=coords;area.onmousedown=function(){emu.setONKeyPressed();}
...
function reset(){for(var p=0;p<calcscreen.length;calcscreen[p++]=0x50){};}
function setEmu(newemu){emu=newemu;}
function setLink(newlink){link=newlink;}
...
function initscreen(){var elem=document.getElementById(elementid_screen);context=elem.getContext('2d');if(screen_scaling_ratio==1){if(context.createImageData)
...
set_skin();initscreen();}
function getPNG(){var data=document.getElementById(elementid_screen).toDataURL('image/png');document.getElementById(elementid_pngimage).src=data;document.getElementById(elementid_pngbutton).style.display='none';document.getElementById(elementid_pngimage).style.display='inline';document.getElementById(elementid_hidebutton).style.display='inline';}
function pngButtons(){document.getElementById(elementid_pngimage).style.display='none';document.getElementById(elementid_hidebutton).style.display='none';document.getElementById(elementid_pngbutton).style.display='inline';}
function pause_emulator(){emu.pause_emulator();document.getElementById(elementid_pauseemulator).style.display='none';document.getElementById(elementid_resumeemulator).style.display='inline';}
function resume_emulator(){emu.resume_emulator();document.getElementById(elementid_pauseemulator).style.display='inline';document.getElementById(elementid_resumeemulator).style.display='none';}
function loadrom(){var infile=document.getElementById(elementid_romfile).files[0];emu.loadrom(infile);}
function set_title(title){document.title=title;}
function getFileData(blob){var url=stdlib.URL.createObjectURL(blob);var a=document.querySelector("#"+elementid_downloadfile);a.href=url;a.download=link.link_recv_foldername()+"."+link.link_recv_varname()+link.buildFileExtensionFromVartype();a.style.display='inline';}
function set_colors_according_to_contrast(){if(emu.hardware_model()==1){black_color=white_color-5*contrast;}
else{black_color=(2*white_color-5*contrast)>>1;}}
function set_screen_enabled_and_contrast(calculator_model,hardware_model,port_60001C,port_60001D,port_70001D,port_70001F){if(hardware_model==1){var new_screen_enabled=((port_60001D&0x10)==0x00)||((port_60001C&0x3C)!=0x3C);if(new_screen_enabled^screen_enabled){stdlib.console.log("Changing screen state: "+new_screen_enabled+"\tpc="+emu.to_hex(emu.pc(),6)+"\thardware_model="+hardware_model+"\t60001C="+emu.to_hex(port_60001C,2)+"\t60001D="+emu.to_hex(port_60001D,2)+"\t70001D="+emu.to_hex(port_70001D,2)+"\t70001F="+emu.to_hex(port_70001F,2));}
...
function updateSkinZoom(val){var div=document.getElementById("calccontainer");div.setAttribute("style","zoom:"+val+"; -moz-transform: scale("+val+");")}
