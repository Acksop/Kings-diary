...
state.rom=false;
state.ram=new Uint16Array(131072);
var cpu=new Object();
cpu.t=new Array(65536);
cpu.n=new Array(65536);
cpu.cycles=new Uint8Array(65536);
state.unhandled_count=0;
var main_interval_timer_id=0;
state.main_interval_timer_interval=11;
state.tracecount=0;
state.cycle_count=0;
state.overall=2500;
...
var ui=false;var link=false;
state.erase_ram_upon_reset=true;
state.port_600000=0x04;
state.vectorprotect=false;
state.wakemask=0;
state.lcd_address_high=9;
state.lcd_address_low=0x80;
state.lcd_address=0x4c00;
state.screen_width=240;
state.screen_height=128;
state.interrupt_control=0x1B;
state.interrupt_rate=0x200;
state.timer_min=0xB2;
state.timer_current=0;
state.keystatus=new Uint8Array(80);
state.keymaskhigh=0xFF;
state.keymasklow=0xFF;
state.port_60001A=0x02;
state.port_60001B=0;
state.port_60001C=0x01;
state.port_60001D=0x8D;
state.port_700017=0;
state.port_70001D=0;
state.port_70001F=0;
state.stopped=false;
...
ui.setCalculatorModel(state.calculator_model);link.setCalculatorModel(state.calculator_model);ui.initemu();initialize_calculator();main_interval_timer_id=stdlib.setInterval(emu_main_loop,state.main_interval_timer_interval);for(var key=0;key<80;key++)state.keystatus[key]=0;ui.initkeyhandlers();};
function setKey(keynumber,status){state.keystatus[keynumber]=status;}
...
{
var buf;
if(typeof(newfileready)=="object"){
	if(newfileready instanceof Array){
		buf = newfileready;
	}else if(newfileready instanceof FileReader){
		buf = new Uint8Array();
		buf.from( newfileready.result );
	}else{
		buf = new Uint8Array();
		buf.from( newfileready.result );
	}
...
if(buf[0x49]==1){
	vartype=0x26;
}else if(buf[0x49]==2||buf[0x49]==3){
	vartype=0x27;
}
var data_len=buf[0x57]+buf[0x56]*256;
link.sendfile(varname,vartype,buf,data_len,0x58,true);
}
...
newflashfileready=false;var varname=new Array();
...
link.sendfile(varname,vartype,buf,data_len,0x4E,false);
}
...
function loadrom(infile)
{
	...
	if(infile.size>=80&&(".9xk.89k.v2k".indexOf(extension)!=-1))
	{
...
{
stdlib.console.log("Starting to autoload as variable");newfileready=reader;state.unhandled_count=0;handle_newfileready();
}
...
setLink:setLink,
loadrom:loadrom,
loadAutoMaticFileReader:loadAutoMaticFileReader, //Addition by Acksop
setKey:setKey,
...
sr:get_sr,pc:get_pc,dn:dn,an:an,
...
set_sr:set_sr,set_pc:set_pc,rom:get_rom,ram:get_ram,t:get_t,n:get_n,cycles:get_cycles,
rb:get_rb,rw:get_rw,rl:get_rl,wb:get_wb,ww:get_ww,wl:get_wl,
rb_1_normal:get_rb_1_normal,rb_1_flashspecial:get_rb_1_flashspecial,
rw_1_normal:get_rw_1_normal,rw_1_flashspecial:get_rw_1_flashspecial,
wb_1_normal:get_wb_1_normal,
ww_1_normal:get_ww_1_normal,ww_1_flashspecial:get_ww_1_flashspecial,
rb_3_normal:get_rb_3_normal,rb_3_flashspecial:get_rb_3_flashspecial,rw_3_normal:get_rw_3_normal,rw_3_flashspecial:get_rw_3_flashspecial,
wb_3_normal:get_wb_3_normal,ww_3_normal:get_ww_3_normal,ww_3_flashspecial:get_ww_3_flashspecial,
rb_8_normal:get_rb_8_normal,rb_8_flashspecial:get_rb_8_flashspecial,rw_8_normal:get_rw_8_normal,rw_8_flashspecial:get_rw_8_flashspecial,
wb_8_normal:get_wb_8_normal,ww_8_normal:get_ww_8_normal,ww_8_flashspecial:get_ww_8_flashspecial,
rb_9_normal:get_rb_9_normal,rb_9_flashspecial:get_rb_9_flashspecial,rw_9_normal:get_rw_9_normal,rw_9_flashspecial:get_rw_9_flashspecial,
wb_9_normal:get_wb_9_normal,ww_9_normal:get_ww_9_normal,ww_9_flashspecial:get_ww_9_flashspecial,
stopped:get_stopped,hardware_model:get_hardware_model,calculator_model:get_calculator_model,jmp_tbl:get_jmp_tbl,
ROM_base:get_ROM_base,FlashMemorySize:get_FlashMemorySize,toggle_framesync:toggle_framesync,
_save_state:_save_state,_restore_state:_restore_state
}
;}*/
...
else if(screen_scaling_ratio==4){output_calcscreen_to_bitmap_scale4(calcscreen,bitmap.data);context.putImageData(bitmap,0,0);}};
...
getPNG:getPNG,
loadrom:loadrom,
loadautofile:loadautofile, //addition by Acksop
